...
public:
  ...
  virtual void query(void *source_creature, StatQuery &s_query) = 0;
};
...
public:
  ...
  void query(void *source_creature, StatQuery &s_query) override
  {
    if (source_creature == this)
    {
      switch (s_query.statistic)
      {
      case StatQuery::attack:
        s_query.result += base_attack;
        break;
      case StatQuery::defense:
        s_query.result += base_defense;
        break;
      }
    }
    else
    {
      if (s_query.statistic == StatQuery::defense)
      {
        s_query.result++;
      }
    }
